...
Usage:
    ...
    uv run python -m mycode.mcp_server.server

    # Or via the CLI command:
    uv run mycode mcp-server
"""
...
async def send_message(
    ...
    This is the core interaction tool. The mycode agent will process your message,
...
async def server_status() -> str:
    ...
    async with _client() as client:
        try:
            ...
            return json.dumps({
                "status": "offline",
                "error": f"Cannot connect to mycode server at {OPENCODE_BASE_URL}. "
                         "Please start it with: uv run mycode serve",
            }, ensure_ascii=False)
