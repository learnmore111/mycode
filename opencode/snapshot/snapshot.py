...
class Snapshot:
    ...
    async def diff(self, tree_hash: str) -> str:
        ...
        code, text, _ = await _git(
            ["--git-dir", self.gitdir, "--work-tree", self.worktree, "diff", "--cached", "--no-ext-diff", tree_hash, "--", "."],
            cwd=self.worktree)
    ...
    async def patch(self, tree_hash: str) -> dict:
        ...
        await _git(["--git-dir", self.gitdir, "--work-tree", self.worktree, "add", "--sparse", "."],
                    cwd=self.worktree)
        code, text, _ = await _git(
            ["--git-dir", self.gitdir, "--work-tree", self.worktree, "diff", "--cached", "--no-ext-diff", "--name-only", tree_hash, "--", "."],
            cwd=self.worktree)
